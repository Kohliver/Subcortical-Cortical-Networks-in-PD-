...
Sub-RSN Project - Preprocessing 8

    Align the sign of each parcel time course across subjects
    and save the data as a vanilla numpy file.
    This resolves sign ambiguity of beamforming to avoid that phase relationships
    are cancelled out at the groop level.
...
# Get path and filenames for PD patients in peri and HC conditions
files = []
for condition in ['peri','HC']:
    files.extend(sorted(glob(f"{src_dir}/*{condition}*/parc/parc-raw.fif")))
...
# Load template covariance - (is taken from canonical state descriptions used for HMM fitting)
template_cov = np.load(".../data/hmm/8_states_norm_model/public/template_cov.npy")
...
for i in range(len(files)):
    ...
    outdir = os.path.dirname(os.path.dirname(files[i]))
