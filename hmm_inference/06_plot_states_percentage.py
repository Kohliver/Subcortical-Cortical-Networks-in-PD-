...
def tsplot(ax, data, time, color_data = 'blue', linestyle = 'solid', linewidth = 2):
    x = time
    ...
    # Make Axes pretty
    ax.set_xlim([2, 30])
    ax.set_ylim([-60,175]) # 6 = .041, 8= .063, 10 = .06
    ax.set_xlabel('Frequency (Hz)',fontsize=20, labelpad=12)
    ax.set_ylabel('Power Change (%)', fontsize=20)
...
# Set Dirs
indir = '.../data/hmm/post_hoc/8_states_norm/inf' 
output_dir ='.../results/hmm/post_hoc/8_states_norm/state_descriptions'
...
for i in range(gpsd.shape[1]):
    ...
    # Make Plot
    fig, ax = plt.subplots()
    tsplot(ax, gpsd[:,i,17:19].mean(axis=1), time=freqs, color_data=col[1])
    ax.axhline(0,linestyle='--',color='k', alpha=.7)
    
    #Save Figure
    plt.savefig(output_dir +  f"/all_percentage/motor_psd_{i}.svg",
...
for i in range(p.shape[0]):
    ...
    power.save(
        ...
        component=0,
        subtract_mean=True,
        plot_kwargs={"cmap": "RdBu_r", "bg_on_data": True}, #"darkness": 0.8, "alpha": 1,"vmin": -ymax, "vmax": ymax},
        #filename=f"{output_dir}/all_percentage/pow_{i}.svg",
    )
...
gcoh = np.zeros(coh.shape)
for iSub in range(psd.shape[0]):
    mean_coh = np.average(coh[iSub], axis=0, weights=fo[iSub])
    gcoh[iSub] = (coh[iSub] - mean_coh) / mean_coh * 100
...
connectivity.save(
    ...
    filename=output_dir + "/all_percentage/coh_.svg",
)
