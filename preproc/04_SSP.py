...
Sub-RSN Project - Preprocessing 4

    Run SSP and remove manually selected projections (previous step) from all
    files. Power Spectra of cleaned data are plotted.
    
    Prior to running this script, differen number of projections were removed
    and the number best cleaning the data, while not removing artefact unrelated
    data was selected. The participants optimal number of projections are stored
    in excel file and loaded in this script prior to applying the cleaning.
"""
...
report_dir = ".../preproc_ssp/report/proj1"
...
for condition in ['peri','HC']:
    preproc_files.extend(sorted(glob(f".../data/preproc/*{condition}*/*_preproc_raw.fif")))

# Load csv with nproj per participant (make sure that order matches file order in preproc_files)
ecg_nprojs = np.genfromtxt('.../results/preproc/all_nproj.csv', delimiter=',', dtype=int)
    
# Exclude a few participants marked in ecg_nprojs as np.nan or -1 - most excluded because of poor data quality
# loop through indices backwards and remove bad data sets
...
for file in preproc_files:
        ...
        ses = base[1]
        run = base[3]
        ssp_preproc_files.append(f"{ssp_preproc_dir}/{subject}/{subject}_{ses}_{run}_preproc_raw.fif")      
        plot_files.append(f"{report_dir}/{subject}/{subject}_{ses}_{run}")
